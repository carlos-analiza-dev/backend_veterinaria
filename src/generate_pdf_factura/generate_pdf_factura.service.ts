...
export class FacturaPdfService {
  constructor(
    ...
    private readonly facturaRepository: Repository<AgroFacturacion>,
  ...
  async generarAgroFacturaPDF(
    ...
    const simbolo = agroserivio.pais.simbolo_moneda ?? '$';
    ...
    try {
      ...
      const datosEmpresa = await this.datosAgroRepository.findOne({
        where: { id: agroserivio.id },
      });
      ...
      const url = new URL(agroserivio.logo.url);
      ...
      factura.detalles.forEach((detalle, index) => {
        ...
        const backgroundColor = index % 2 === 0 ? '#FFFFFF' : '#F8F9FA';
        const descripcion = detalle.producto?.nombre || 'Producto';
