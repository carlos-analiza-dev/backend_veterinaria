...
export class NotaCreditoPdfService {
  constructor(
    ...
    private readonly agroNotaCreditoRepository: Repository<AgroNotaCredito>,
  ...
  async generarAgroNotaCreditoPDF(
    ...
    let notaCredito: AgroNotaCredito;
    let datosAgroservicio: DatosAgroservicio;

    try {
      notaCredito = await this.agroNotaCreditoRepository.findOne({
        where: { id },
        relations: [
          'factura',
          'factura.cliente',
      ...
      if (!datosAgroservicio) {
        return res
          .status(404)
          .json({ message: 'Datos del agroservicio no configurados' });
      }
    } catch (validationError) {
      return res.status(500).json({
        message: 'Error al validar datos',
    ...
    try {
      ...
      doc.on('error', (pdfError) => {
        console.error('Error en PDFKit:', pdfError);
      ...
      doc
        .font('Helvetica-Bold')
        .text(
          `Propietario: ${datosAgroservicio.nombre_agroservicio}`,
          50,
          infoEmpresaY,
        );
      doc
        .font('Helvetica')
        .text(datosAgroservicio.direccion, 50, infoEmpresaY + 12);
      doc.text(`${datosAgroservicio.correo}`, 50, infoEmpresaY + 24);
      doc.text(`RTN: ${datosAgroservicio.rtn}`, 50, infoEmpresaY + 36);
      ...
      console.error('Error al generar la nota de crédito PDF:', pdfError);
  ...
  async generarAgroNotaCreditoPreview(
    ...
    try {
      return this.generarAgroNotaCreditoPDF(id, propietarioId, res, true);
    } catch (error) {
      if (!res.headersSent) {
        res.status(500).json({
          message: 'Error al generar la previsualización',
