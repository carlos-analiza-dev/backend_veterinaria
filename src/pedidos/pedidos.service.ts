...
export class PedidosService {
  ...
  try {
    ...
    if (createPedidoDto.total && Math.abs(totalCalculado - createPedidoDto.total) > 0.01) {
      throw new BadRequestException(
...
async update(
  ...
  try {
    ...
    if (pedido.estado !== EstadoPedido.PENDIENTE && updatePedidoDto.detalles) {
      throw new BadRequestException(
    ...
    if (updatePedidoDto.detalles && updatePedidoDto.detalles.length > 0) {
      ...
      for (const oldDetalle of pedido.detalles) {
        if (oldDetalle.id_sucursal) {
          ...
          for (const lote of lotes) {
            ...
            const cantidadLote = Number(lote.cantidad);
            const cantidadADevolver = Math.min(cantidadLote, cantidadPorDevolver);
      ...
      for (const detalleDto of updatePedidoDto.detalles) {
        ...
        for (const lote of lotes) {
          ...
          const cantidadLote = Number(lote.cantidad);
          const cantidadADescontar = Math.min(cantidadLote, cantidadPorDescontar);
    ...
    return await this.pedido_repo.findOne({
      where: { id },
      relations: ['cliente', 'detalles', 'detalles.producto', 'detalles.producto.tax'],
    });
  } catch (error) {
    await queryRunner.rollbackTransaction();
